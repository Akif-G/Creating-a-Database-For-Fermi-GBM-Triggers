...
heasarc["trigger_time"] = pd.to_numeric(
    heasarc["trigger_time"], errors='ignore')
...
try:
    ...
    mode1.drop(['nearest', 'nearest(abs)', 'nearest_name',
                'heasarc'], axis=1, inplace=True)
        ...
        if hea_trig.at[i] >= (mode1_trig.at[j]-0.5):
            if i > 0:
                if abs(hea_trig.at[i]-mode1_trig.at[j]) < abs(hea_trig.at[i-1]-mode1_trig.at[j]):
                    mode1.at[j, 'nearest'] = (hea_trig.at[i]-mode1_trig.at[j])
                    mode1.at[j, 'nearest(abs)'] = abs(
                        hea_trig.at[i]-mode1_trig.at[j])
                    mode1.at[j, 'nearest_name'] = str(
                        heasarc.at[i, 'trigger_name'])
                else:
                    mode1.at[j, 'nearest'] = (
                        hea_trig.at[i-1]-mode1_trig.at[j])
                    mode1.at[j, 'nearest(abs)'] = abs(
                        hea_trig.at[i-1]-mode1_trig.at[j])
                    mode1.at[j, 'nearest_name'] = str(
                        heasarc.at[i-1, 'trigger_name'])
            if hea_trig.at[i] <= (mode1_trig.at[j]+0.5):
...
try:
    ...
    mode2.drop(['nearest', 'nearest(abs)', 'nearest_name',
                'heasarc'], axis=1, inplace=True)
        ...
        if hea_trig.at[i] >= (mode2_trig.at[j]-0.5):
            if i > 0:
                if abs(hea_trig.at[i]-mode2_trig.at[j]) < abs(hea_trig.at[i-1]-mode2_trig.at[j]):
                    mode2.at[j, 'nearest'] = (hea_trig.at[i]-mode2_trig.at[j])
                    mode2.at[j, 'nearest(abs)'] = abs(
                        hea_trig.at[i]-mode2_trig.at[j])
                    mode2.at[j, 'nearest_name'] = str(
                        heasarc.at[i, 'trigger_name'])
                else:
                    mode2.at[j, 'nearest'] = (
                        hea_trig.at[i-1]-mode2_trig.at[j])
                    mode2.at[j, 'nearest(abs)'] = abs(
                        hea_trig.at[i-1]-mode2_trig.at[j])
                    mode2.at[j, 'nearest_name'] = str(
                        heasarc.at[i-1, 'trigger_name'])
...
try:
    ...
    mode3.drop(['nearest', 'nearest(abs)', 'nearest_name',
                'heasarc'], axis=1, inplace=True)
        ...
        if hea_trig.at[i] >= (mode3_trig.at[j]-0.5):
            if i > 0:
                if abs(hea_trig.at[i]-mode3_trig.at[j]) < abs(hea_trig.at[i-1]-mode3_trig.at[j]):
                    mode3.at[j, 'nearest'] = (hea_trig.at[i]-mode3_trig.at[j])
                    mode3.at[j, 'nearest(abs)'] = abs(
                        hea_trig.at[i]-mode3_trig.at[j])
                    mode3.at[j, 'nearest_name'] = str(
                        heasarc.at[i, 'trigger_name'])
                else:
                    mode3.at[j, 'nearest'] = (
                        hea_trig.at[i-1]-mode3_trig.at[j])
                    mode3.at[j, 'nearest(abs)'] = abs(
                        hea_trig.at[i-1]-mode3_trig.at[j])
                    mode3.at[j, 'nearest_name'] = str(
                        heasarc.at[i-1, 'trigger_name'])
...
try:
    ...
    mode4.drop(['nearest', 'nearest(abs)', 'nearest_name',
                'heasarc'], axis=1, inplace=True)
        ...
        if hea_trig.at[i] >= (mode4_trig.at[j]-0.5):
            if i > 0:
                if abs(hea_trig.at[i]-mode4_trig.at[j]) < abs(hea_trig.at[i-1]-mode4_trig.at[j]):
                    mode4.at[j, 'nearest'] = (hea_trig.at[i]-mode4_trig.at[j])
                    mode4.at[j, 'nearest(abs)'] = abs(
                        hea_trig.at[i]-mode4_trig.at[j])
                    mode4.at[j, 'nearest_name'] = str(
                        heasarc.at[i, 'trigger_name'])
                else:
                    mode4.at[j, 'nearest'] = abs(
                        hea_trig.at[i-1]-mode4_trig.at[j])
                    mode4.at[j, 'nearest'] = (
                        hea_trig.at[i-1]-mode4_trig.at[j])
                    mode4.at[j, 'nearest(abs)_name'] = str(
                        heasarc.at[i-1, 'trigger_name'])
